...
components (same `type` names, same prop names/semantics — OpenFused implements *fewer*
props, never extra). Several are OpenFused-owned primitives **not** governed by app parity:
...
OpenFused's own `dropdown` conventions), `task-board` (the task surface — list + kanban +
...
| [`checkbox-group`](./widgets/checkbox-group.md) | multi-select checkboxes; writes an **ARRAY** param (OpenFused-owned, no app equivalent) | `param`, `options` / `sql`, `defaultSelected`, `minSelected`, `maxSelected` |
...
| [`canvas`](./widgets/canvas.md) | free-form layout: nodes are widgets, wired by edges carrying param dataflow (OpenFused-owned; [`canvas.md`](./canvas.md)) | `nodes` (`[].widget`), `edges` | no |
...
### Feedback primitives (OpenFused-owned — not governed by app parity)
...
Actions & selection. `canvas` (in Containers above) is the third OpenFused-owned primitive, and
`task-board` (below) the fourth.

### Task + agent surfaces (OpenFused-owned — not governed by app parity)
...
components: same `type` names, same prop names/semantics, OpenFused implements *fewer* props
and never extra. A config authored against these pastes into the application and behaves
identically. Four components are OpenFused-owned and **not** governed by app parity:
...
- **`checkbox-group`** — the array-writing multi-select input. The application's only selection
  ...
  `checkbox-group` aligns its props to OpenFused's own `dropdown`, not to an app component.
