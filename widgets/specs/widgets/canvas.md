...
- Each `nodes[]` entry becomes a `json-ui` ReactFlow node (an fused `RenderNode`), rendered under an edge-gated per-node bridge from the canvas runtime so each node POSTs to the SAME host resolve URL. The canvas reads its data plane (config/data/errors/depMap/resolve URL, plus host flags) from `CanvasHostContext`, published by the bundle host — NOT from the SDK `element` (the renderer hands a widget only its `element`).
